	...
	*/
	...
	{	
		...
		if ($this->_debug) {
			$this->_ticker++;
			$this->_log[] = $sql;
	        mysql_query("SET profiling_history_size=1");
		...
		if( $this->_debug){
			$lastAffectedRows = mysql_affected_rows();
			$profile = array();
		    if ($profileQ = mysql_query("SHOW PROFILES")){
                 $profileData = array();
                 if ($profileItem = mysql_fetch_assoc($profileQ)){
                    $details = $this->_getProfileDetails($profileItem['Query_ID']);
					$profileDetailTable = array($sql, $details[0]); 
                 }
         	}
         	$this->_lineQuery[] = $profileDetailTable;
         	$this->_totalDurationQuery += $details[0];
		}
 	...
 	function _getProfileDetails($queryId){
         $rsd = mysql_query("SELECT 
         	round(sum(duration), 5) as `sum_dur`
         FROM information_schema.profiling 
         WHERE `query_id` = '{$queryId}' ");
         $rsv = array();
         $dur = $cpu = 0;
	     while($rdd = mysql_fetch_assoc($rsd)){
         	$dur       += $rdd['sum_dur'];
	     }
         return array($dur);
 	}
	 ...
	 * с использованием memcache
	 * @param string $defaultMethod   метод, который возвращает данные из БД, если их нет в кеше memcache
	 * @param int    $expiration      время жизни кеша в сек
	 * @return array
	 */
	function loadCache($defaultMethod='loadObjectList', $expiration=0)
	{
		$ret = null;
		if( MEMCACHE_ACTIV ){
			if( !$this->memcache ){
			...
			$sql = md5($this->_sql);
			$ret = $this->memcache->get("sql:{$sql}");
			if( !$ret ){
				$ret = $this->$defaultMethod();
				$this->memcache->set("sql:{$sql}", $ret, false, $expiration);
			}else{
				$this->_sqlFromCache++;
			}
		}else{
