...
class JDatabase extends JObject
{
	...
	var $_lineQuery          = array();
	var $_totalDurationQuery = 0;
	var $_arrayForCheck      = array();
	var $_alertQuery         = array();
	...
	*/
	...
	/**
	 * возвращает таблицу логов firePHP
	 * @return string
	 */
	function FirePhpLog(){
		$table   = array();
		$table[] = array('num', 'sql', 'duration, sec');
		if( is_array($this->_lineQuery) ){
			$i = 1;
			foreach( $this->_lineQuery as $q ){
				$this->_getQueryCount($q[0]);
				$table[] = array($i, $q[0], $q[1]);
				$i++;
			}
		}
		return $table;
	...
	/**
	 * возвращает общее время работы MySQL сервера
	 */
	function getTotalDurationQuery(){
		return $this->_totalDurationQuery;
	}
	/**
	 * возвращает общее кол-во запрос, взятых с кеша Memcache
	 * @return int
	 */
	function getTotalSqlFromCache(){
		return $this->_sqlFromCache;
	}
	/**
	 * запоминает запросы для дальнейшей проверки повторов
	 * @param string $query SQL запрос
	 */
	function _getQueryCount($query){
		$this->_arrayForCheck[] = $query;
	}
	/**
	 * возвращает список повторяющихся запорсов
	 * @return array
	 */
	function getAlertQuery(){
		$return = array();
		$array = array_count_values($this->_arrayForCheck);
		if( $array ){
