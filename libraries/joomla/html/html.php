...
class JHTML
{
	...
	function superImg($folder, $image, $attribs = null, $title = null) {
		$base = JURI::base();
		if ($attribs) {$save = (SAVE_IMAGES) ? '' : '&ns';}
		return "<img title='{$title}' src='{$base}images/sunny/{$folder}/{$image}{$attribs}{$save}' />";
	}
	
	function youTubeImg($key, $width, $height) {
		return "<img width='{$width}' height='{$height}' src='http://i.ytimg.com/vi/{$key}/0.jpg' />";
	}
	...
	*/
		function pagination($page=1, $all, $limit=1, $option)
	{
		$html = '';
		if( ceil($all/$limit) > 1 ):
		$html .= '<div align="center" class="club_news_all_pager"><form action="'.JRoute::_('index.php?'.$option).'" method="post">';
			...
			if ($page != 1):
				$pervpage = '<a style="margin: 10px 10px" href="'.$page1left_link .'" title="На страницу '.($page-1).'"></a>';
			endif;
			if ($page != $total):
				$nextpage = '<a  href="'.$page1right_link.'" title="На страницу '.($page+1).'"></a>';
			endif;
			if ($page == $total):
				$lastpage = '<a style="margin: 10px 10px" href="'.$lastpage_link.'" title="На страницу "></a>';
			endif;
			$page2left = ( $page - 2 > 0 )
								...
								;
			$page1left = ( $page - 1 > 0 ) 
							 ? '<a style="margin: 10px 10px" href="'.$page1left_link.'" title="На страницу '.($page-1).'">'.($page-1).'</a>'
							 : ''
							 ;
			...
			$page1right = ( $page + 1 <= $total )
								? '<a style="margin: 10px 10px" href="'.$page1right_link.'" title="На страницу '.($page+1).'">'.($page+1).'</a>'
								: ''
								;	
            $start=		( $page + 1 <= $total )	
								? ''
								: '<a style="margin: 10px 10px" href="" >&larr; <span>В начало</span></a>'
								;	
            $last=		( $page - 1 > 0 )	
								? ''
								: '<a style="margin: 10px 10px" href="'.$lastpage_link.'" ><span>В конец</span> &rarr;</a>'
								;									
			$html .= $start 
					 .$pervpage
					 . $page2left
	...
	function getDateStr($date) {
		...
		$sCommentDate = $date;
		$aCommentDate = explode("-", $sCommentDate);
	
		$aCommentTime = explode(" ", $aCommentDate[2]);
						
		if ($aDate[0] == $aCommentDate[0] 
			  && $aDate[1] == $aCommentDate[1] 
			    && $aDate[2] == $aCommentTime[0]) {
			$strDate = "сегодня {$aCommentTime[1]}";
		} else if ($aDate[0] == $aCommentDate[0] 
	  		&& $aDate[1] == $aCommentDate[1] 
	    	  && $aDate[2] == ($aCommentTime[0] + 1)) {
			$strDate = "вчера {$aCommentTime[1]}";
		} else {
			$strDate = JHTML::date($date, '%d %B в %H:%M:%S');
		}
