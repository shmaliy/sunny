...
class ContentViewFrontpage extends JView
{
	function display($tpl = null)
	{
		...
		foreach ( $rows as $row )
		{
			$link               = JURI::base()."{$menulinkNews}consulting/club/{$row->alias}/";								
			$item = new JFeedItem();
		...
		$q = " SELECT * FROM `#__work` as `work` ";
		$q .= " WHERE published = '1' ";
		$q .= " ORDER BY id_work DESC ";
		$db->setQuery($q);
		$rows = $db->loadObjectList();
		foreach ( $rows as $row )
		{
			$link               = JURI::base()."{$menulinkNews}portfolio/{$row->alias}/";								
			$item = new JFeedItem();
			$item->title 		= $row->title;
			$item->link 		= $link;
