...
class PartnersModelPartners extends JModel{
	...
	* сохраняет запись
	*/
	function _save($title, $alias, $img, $href, $type, $published){
		$db = &JFactory::getDBO();
		$q = " INSERT INTO `{$this->table}` VALUES('', '{$title}', '{$alias}', '{$img}', '{$href}', '{$type}', '{$published}') ";
		$db->setQuery($q);
		$db->query();		
	}
	/**
	...
	*/
	function _update($id, $title, $alias, $img, $href, $type, $published){
		$db = &JFactory::getDBO();
