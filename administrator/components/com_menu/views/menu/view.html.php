...
class MenuViewMenu extends JView{
	function display($tpl = null){
		...
		$lists['order']	     = $mainframe->getUserStateFromRequest( "com_menu.category.filter_order",     'filter_order',     'c.ordering', 'cmd' );
		$lists['order_Dir']	 = $mainframe->getUserStateFromRequest( "com_menu.category.order_Dir", 'filter_order_Dir', 'ASC',        'word' );			
		
		$limit		= $mainframe->getUserStateFromRequest( 'global.list.limit',      'limit',      $mainframe->getCfg('list_limit'), 'int' );
		$limitstart = $mainframe->getUserStateFromRequest( 'com_menu.limitstart', 'limitstart', 0,                                'int' );
