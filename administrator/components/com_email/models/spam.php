...
class EmailModelSpam extends JModel{
	/**
	* @var string $table таблица с предоставляемыми услугами
	*/	
	var $table = '#__as_email';
	/**
...
/**
	...
	* сохраняет запись
	*/
	function _save($fio, $email,  $activity, $spam_avto, $spam_manual, $comments, $published, $ordering){
		$db = &JFactory::getDBO();
		$q = " INSERT INTO `{$this->table}` VALUES('', '{$fio}', '{$email}',  '{$activity}', '{$spam_avto}', '{$spam_manual}', '{$comments}', '{$published}', '{$ordering}') ";
		$db->setQuery($q);
		$db->query();		
	}
	/**
	...
	*/
	function _update($id, $fio, $email, $activity, $spam_avto, $spam_manual, $comments, $published, $ordering){
		$db = &JFactory::getDBO();
