...
class ServicesModelLibrary extends JModel{
	/**
	...
	* сохраняет запись
	*/
	function _save($title, $href, $size, $desc, $autor, $published){
		$db = &JFactory::getDBO();
		$q = " INSERT INTO `{$this->table}` VALUES('', '{$title}', '{$desc}', '{$href}', '', '{$size}',  '{$autor}', '{$published}') ";
	...
	*/
	function _update($id, $title, $href, $size, $desc, $autor, $published){
		$db = &JFactory::getDBO();
