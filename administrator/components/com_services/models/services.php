...
class ServicesModelServices extends JModel{
	...
	* сохраняет запись
	*/
	function _save($id_category, $title, $alias, $s_desc, $desc, $published, $ordering){
		$db = &JFactory::getDBO();
		$q = " INSERT INTO `{$this->table}` VALUES('', '{$id_category}', '{$title}', '{$alias}', '{$s_desc}', '{$desc}', '{$published}', '{$ordering}') ";
		$db->setQuery($q);
		$db->query();		
	}
	/**
	...
	*/
	function _update($id, $id_category, $title, $alias, $s_desc, $desc, $published, $ordering){
		$db = &JFactory::getDBO();
