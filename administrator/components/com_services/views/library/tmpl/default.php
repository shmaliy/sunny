<?php
	...
	$i=0;
?>
<form action="index.php?option=<?php echo $option?>&view=library" method="post" name="adminForm">
<div id="editcell">
	<table class="adminlist">
		<thead>
			<tr>
                ...
                <th width="350"><?php echo JText::_('Ссылка')?></th>
                <th width="80">
		...
		<tbody>
			<?php 
				foreach($this->items as $item):
						$id = $item->id;
						$i++;
						$link_edit = JRoute::_("index.php?option={$option}&view=library&layout=form&task=edit&id={$id}");
						if( $item->published == 0 ):
							$link_pub = JRoute::_("index.php?option={$option}&view=library&controller={$controller}&task=publish&cid[]={$id}");
							$link_pub = "<a href=\"{$link_pub}\" alt=\"Опубликовать\"><img src='images/publish_x.png' /></a>";						
						else:
							$link_pub = JRoute::_("index.php?option={$option}&view=library&controller={$controller}&task=unpublish&cid[]={$id}");
					...
					<tr class="row<?php echo ($i%2) ? 0 : 1;?>">
						...
						<td align="center"><?php echo $item->href?></td>
