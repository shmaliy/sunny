...
* @subpackage Clients
*/
class ClientsControllerCategoryt extends ClientsController{
	/**
	* @var string $view вид
	*/	
	var $view      = 'categoryt';
	...
	*/
	...
	function __construct(){
		...
		$this->model = $this->getModel('Categoryt');
	}
	/**
	...
	*/	
	function save(){
		...
		if( $image['name'] && $image['name'] != '' ):
			...
			$tmp_dest 	= JPath::clean(JPATH_ROOT.DS.'images\sunny\catalogt').DS.'cl_'.$time.$ext;
		...
		if( !$id ):
			$this->model->_save( $alias, $title, $desc, $img, $published);
			$this->setRedirect("index.php?option={$this->component}&view={$this->view}", JText::_('Изменения сохранены'));
		else:
			$this->model->_update($id, $title, $alias, $desc, $img, $published);
			$this->setRedirect("index.php?option={$this->component}&view={$this->view}", JText::_('Изменения сохранены'));
		endif;
