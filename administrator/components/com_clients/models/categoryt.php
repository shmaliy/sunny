...
class ClientsModelCategoryt extends JModel{
	/**
	...
	* сохраняет запись
	*/
	function _save($title, $alias, $desc,  $img, $published, $ordering){
		$db = &JFactory::getDBO();
		$q = " INSERT INTO `{$this->tCat}` VALUES('','{$title}', '{$alias}', '{$desc}', '{$img}','{$published}', '{$ordering}') ";
		$db->setQuery($q);
		$db->query();
	}
	/**
	...
	*/
	function _update($id, $title, $alias, $desc, $img, $published){
		$db = &JFactory::getDBO();
