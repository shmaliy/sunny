...
* @subpackage Clients
*/
class ConsultingControllerProjects extends ConsultingController{
	/**
	* @var string $view вид
	*/	
	var $view      = 'projects';
	...
	*/
	...
	function __construct(){
		...
		$this->model = $this->getModel('Projects');
	}
	/**
	...
	*/	
	function save(){
		...
		if( $image['name'] && $image['name'] != '' ):
			...
			$tmp_dest 	= JPath::clean(JPATH_ROOT.DS.'images\sunny\consultant').DS.'p_'.$time.$ext;
		...
		if( !$id ):
			$this->model->_save($id_consultant, $title, $href, $img, $desc, $published);
			$this->setRedirect("index.php?option={$this->component}&view={$this->view}", JText::_('Изменения сохранены'));
		else:
			$this->model->_update($id, $id_consultant, $title, $href, $img, $desc, $published);
			$this->setRedirect("index.php?option={$this->component}&view={$this->view}", JText::_('Изменения сохранены'));
		endif;
